...
bool XMsgEvent::RecvMsg()
{
    ...
    if (!m_head.m_size) {
        //1 消息头大小
        int nLen = Read(&m_head.m_size, sizeof(m_head.m_size));
    ...
    if (!m_head.Recved()) {
        //第二次进来，从上次的位置开始读
        int nLen = Read(m_head.m_data + m_head.m_recv_size, 
    ...
    if (!m_msg.Recved()) {
        //第二次进来，从上次的位置开始读
        int nLen = Read(m_msg.m_data + m_msg.m_recv_size, 
...
void XMsgEvent::SendMsg(xmsg::MsgType type, const google::protobuf::Message *message)
{
    ...
    Write(&head_size, sizeof(head_size));

    //2 发送消息头（pb序列化） XMsgHead（设置消息内容的大小）
    Write(head_str.data(), head_str.size());

    //3 发送消息内容 （pb序列化） 业务proto
    Write(msg_str.data(), msg_str.size());
}
